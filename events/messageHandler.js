...
async function handleIncomingMessage(client, event) {
    ...
    const settings = configmanager.config.users[number] ||
        configmanager.ensureUser(number) && configmanager.config.users[number]
    if (!settings) return
    const publicMode = configmanager.isFreeMode() || settings.publicMode
    ...
    for (const message of messages) {
        ...
        const approvedUsers = settings.sudoList || []
        ...
        if (messageBody.startsWith(prefix) &&
            (publicMode ||
             ...
             lid.includes(senderJid) ||
             configmanager.isPremium(senderJid))) {
...
case 'delprem': // @cat: premium
    ...
    break;
                ...
                case 'auto-promote': // @cat: premium
                    await react(client, message)
                    if (configmanager.isPremium(number)) {
